...
class VenvManager:
    ...
    def __init__(self, venv_base_dir: str = ".venv_plugins"):
        ...
        # 记录已创建的虚拟环境
        self.created_venvs: Dict[str, str] = {}
        ...
        try:
            ...
            try:
                result = subprocess.run(
                    [sys.executable, "-m", "uv", "venv", venv_path],
                    capture_output=True,
                    text=True
                )
            ...
            try:
                if uv_available and has_pyproject:
                    # 使用uv安装依赖
                    ...
                    if has_requirements:
                        ...
                        self.logger.warning("插件目录下没有requirements.txt或pyproject.toml文件，跳过依赖安装")
                        # 没有依赖文件，跳过安装
                        pass
    ...
    def delete_venv(self, plugin_name: str) -> bool:
        ...
        try:
            ...
            # 使用uv删除虚拟环境
            result = subprocess.run(
                [sys.executable, "-m", "uv", "venv", "remove", venv_path],
                capture_output=True,
                text=True
            )
            
            if result.returncode != 0:
                self.logger.error(f"删除插件 {plugin_name} 的虚拟环境失败: {result.stderr}")
                return False
            ...
            try:
                if uv_available and has_pyproject:
                    ...
                    if has_requirements:
                        ...
                        self.logger.warning("插件目录下没有requirements.txt或pyproject.toml文件，跳过依赖更新")
                        # 没有依赖文件，跳过更新
                        pass
